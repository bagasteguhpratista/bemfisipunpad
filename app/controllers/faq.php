<?php
    ...
    class faq extends controller
    {
        ...
        function index(){
            ...
            if(isset($search))$v .= " AND questions LIKE '%".$search."%'";
            ...
            {
                $row['questions'] = admin::limit_text($row['questions'],10);
                $results[] = $row;
            }
...
//                $passwordhash = md5(serialize($password));
        ...
        function delete($idd=null){
            ...
            if($_to == "001"){
                $items  = implode("','", $p_del);
                $sql    = "SELECT id, questions as title FROM ".$var['table'][self::$table]." WHERE id IN ('". $items ."')";
        ...
        function reorder(){
            ...
            if(count($reorder) > 0)
            {
                foreach ($reorder as $idval) { $count ++;
                    $sql = "UPDATE ".$var['table'][self::$table]." SET reorder = " . $count . " WHERE id = '" . $idval ."'";
                    echo $sql;
                    db::execute($sql);
                }
                exit;
