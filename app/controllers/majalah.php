<?php
    ...
    class majalah extends controller
    {
        private $db;
        public static $table = "majalah";
...
//                $passwordhash = md5(serialize($password));
                ...
                if($file_size > 0){
                    $data['file'] = file::save_file('file', $var['v_pdf_path']."/majalah/",$alias);
                }
        ...
        function delete($idd=null){
            ...
            if($_to == "002"){
                ...
                $sql = "SELECT file_pdf FROM ". $var['table'][self::$table] ." WHERE id IN ('". $delid ."')";
                db::query($sql, $rs['row'], $nr['row']);
                while($row=db::fetch($rs['row'])) @unlink($var['v_pdf_path']."/majalah/". $row['file_pdf']);
