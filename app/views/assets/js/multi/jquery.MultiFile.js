...
if (window.jQuery)(function ($) {
				...
				MultiFile.addToList = function (slave, slave_count, files) {
					...
					$.each(files, function (i, file) {
						var v = String(file.name || '' ).replace(/[&<>'"]/g, function(c) { return '&#'+c.charCodeAt()+';'; }),
								...
								withTitle = ($(slave).is('.with-title') ? '<input type="text" class="form-control col-md-7 col-xs-12" name="'+$(slave).attr('name').replace("[]","")+'_title[]" style="left: 40px;"  placeholder="Title Image">' : null),
								withContent = ($(slave).is('.with-content') ? '<textarea class="form-control col-md-7 col-xs-12" style="left: 40px;top: 10px; height: 100px;" name="'+$(slave).attr('name').replace("[]","")+'_content[]" placeholder="Content Image"></textarea>' : null ),
								mp = '<span class="MultiFile-title">'+ n +'</span>',
								label =	$(
										(
											'<span class="MultiFile-label" title="' + t + '">'+
												(MultiFile.preview || $(slave).is('.with-preview') ? ( $.inArray ( ext.toLowerCase(), ["jpg","jpeg","gif","png"] ) > -1 ? p : file.name) : mp )+
												// (MultiFile.preview || $(slave).is('.with-preview') ? p : mp )+
											'</span>'
											+
	...
	*/
	$.fn.MultiFile.options = { //$.extend($.fn.MultiFile, { options: {
		...
		// previews
		preview: false,
		previewCss: 'max-height:300px; max-width:300px;padding:10px;',

		// STRING: collection lets you show messages in different languages
		STRING: {
			remove:'<i style="color:#F44336" class="fa fa-close fa-2x" aria-hidden="true"></i>&nbsp;&nbsp;',
