...
<div class="position-relative row form-group">
    <label for="dynamic_input" class="col-sm-3 col-form-label">Akun Media Sosial</label>
    <div class="col-sm-9" id="dynamictable">
        <div class="col-sm-3 table-form">
            <select name="form_table[]" id="formDynamic" class="select2 form-control">
                ...
                <?php
                $rs['category'] = db::data_record_select("id,title as name","list_sosial_media","1=1 ORDER BY created_at ASC");
                while($row = db::fetch($rs['category'])) $results[] = $row;
...
<div class="position-relative row form-group">
	<div id="dynamicContainerTable" style="width:100%;">
	</div>
</div>
<script>
    $(document).ready(function(){
        ...
        $('#add_table').click(function(){
            i++;
            var addTable;
            addTable = '<div class="row-table-'+i+' col-sm-12 position-relative row form-group"><label for="dynamic_input" class="col-sm-3 col-form-label"></label><div class="col-sm-9" id="dynamictable"><div class="col-sm-3" ><select name="form_table[]" class="select2 form-control "><option value="" disabled="disabled" selected=""><?php echo admin::lang('select');  ?></option>';
            <?php
            $results = [];
                $rs['category'] = db::data_record_select("id,title as name","list_sosial_media","1=1 ORDER BY created_at ASC");
                while($row = db::fetch($rs['category'])) $results[] = $row;
            ...
            <?php } ?>
            addTable += '</select></div><div class="col-sm-7"><input type="text" placeholder="link" id="link" name="link[]" class="form-control"></div><div class="col-sm-1 col-form"><a class="btn btn-danger btn_form_remove" id="remove_form" btn-id-table="'+i+'"><i class="fa fa-trash"></i></a></div></div></div>';
